...
/* DST window crop for adjusting HDMI ouput size */
static struct v4l2_rect  dst_crop;
static int last_dst_w, last_dst_h;
static int dst_crop_loaded = 0;
...
int hdmi_set_dst_crop(int w, int h, int l, int t)
{
    if (last_dst_w <= 0 || last_dst_w > 1920)
        last_dst_w = 1920;
    if (last_dst_h <= 0 || last_dst_h > 1080)
        last_dst_h = 1080;

    if (w < 0 || w < l || w > (last_dst_w >> 1) ||
        h < 0 || h < t || h > (last_dst_h >> 1))
        return -EINVAL;
    ...
    // ALOGV("hdmi_set_dst_crop: config (%d, %d, %d, %d) loaded", w, h, l, t);
    dst_crop_loaded = 1;
    return 0;
}

int hdmi_get_dst_crop(int *w, int *h)
{
    if (w)
        *w = dst_crop.width;
    if (h)
        ...
        int dst_w, int dst_h)
{
...
#else
    if (!dst_crop_loaded) {
        /* Uses same radio for SRC & DST window if NO user config */
        if (dst_w * src_h <= dst_h * src_w) {
            dst_crop.width  = 0;
            dst_crop.left   = 0;
            dst_crop.height = dst_h - ((dst_w * src_h) / src_w);
            dst_crop.top    = dst_crop.height >> 1;
        } else {
            dst_crop.width  = dst_w - ((dst_h * src_w) / src_h);
            dst_crop.left   = dst_crop.width >> 1;
            dst_crop.height = 0;
            dst_crop.top    = 0;
        }
    }
    ...
    vo_param.dst_win.w.left   = dst_crop.left;
    vo_param.dst_win.w.top    = dst_crop.top;
    vo_param.dst_win.w.width  = dst_w - dst_crop.width;
    vo_param.dst_win.w.height = dst_h - dst_crop.height;
#endif

    ALOGV("hdmi_set_v_param L%d: (%d, %d) --> (%d, %d) [%d, %d, %d, %d]",
            layer, src_w, src_h, dst_w, dst_h,
            vo_param.dst_win.w.left, vo_param.dst_win.w.top,
            vo_param.dst_win.w.width, vo_param.dst_win.w.height);
        ...
        int rotVal)
{
    ...
    if ((cur_g2d_address == 0) || (src_y_address != prev_src_addr)) {
        ...
        srcAddr.type	= ADDR_PHYS;
        srcAddr.start	= src_y_address;

        //srcImage = {srcAddr, srcAddr, src_w, src_h, src_w*4, AX_RGB, CF_ARGB_8888};
        //srcRect = {0, 0, src_w, src_h};
        srcImage.width	= src_w;
        srcImage.height	= src_h;
        srcImage.stride	= src_w * 4;
        srcImage.order	= AX_RGB;
        srcImage.fmt	= CF_ARGB_8888;
        srcImage.addr	= srcAddr;
        srcImage.plane2	= srcAddr;
        srcImage.rect.x1= 0;
        srcImage.rect.y1= 0;
        srcImage.rect.x2= src_w;
        srcImage.rect.y2= src_h;
        srcImage.need_cacheopr	= true;

        //dstAddr = {(addr_space)ADDR_PHYS, (unsigned long)dst_addr, dst_w*dst_h*dst_bpp, 1, 0};
        dstAddr.type	= ADDR_PHYS;
        dstAddr.start	= (unsigned long)dst_addr;

        //dstImage = {dstAddr, dstAddr, dst_w, dst_h, dst_w*dst_bpp, AX_RGB, (color_format)dst_color_format};
        //dstRect = {0, 0, dst_w, dst_h};
        dstImage.width	= dst_w;
        dstImage.height	= dst_h;
        dstImage.stride	= dst_w*dst_bpp;
        dstImage.order	= AX_RGB;
        dstImage.fmt	= (color_format)dst_color_format;
        dstImage.addr	= dstAddr;
        dstImage.plane2	= dstAddr;
        dstImage.rect.x1= 0;
        dstImage.rect.y1= 0;
        dstImage.rect.x2= dst_w;
        dstImage.rect.y2= dst_h;
        ...
        static int seq_no = 4096;

        memset(&BlitParam, 0, sizeof(BlitParam));
        BlitParam.op	= BLIT_OP_SRC;
        BlitParam.param.premult	= NON_PREMULTIPLIED;
        BlitParam.param.g_alpha	= 0xff;
        BlitParam.param.dither	= 0;
        BlitParam.param.rotate	= g2d_rotation;
        BlitParam.param.scaling	= Scaling;
        //BlitParam.param.repeat	= { NO_REPEAT, 0 };
        //BlitParam.param.bluscr	= { OPAQUE, 0, 0 };
        BlitParam.param.clipping= dstClip;
        BlitParam.param.solid_color	= 0;
        BlitParam.src	= &srcImage;
        BlitParam.dst	= &dstImage;
        BlitParam.msk	= NULL;
        BlitParam.tmp	= NULL;
        BlitParam.sync	= BLIT_SYNC;		/* TODO: or ASYNC ? */
        BlitParam.seq_no= seq_no++;

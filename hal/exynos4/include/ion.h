...
/* See linux-3.0.86/include/linux/ion.h */
 ...
 */
int ion_msync(ion_client client, ion_buffer buffer,
        long flags, size_t size, off_t offset);

/* ion_getphys() - Gets physical address of the buffer identied by @buffer
 * @client: A valid ion_client value returned by ion_client_create().
 * @buffer: The buffer to perform ion_getphys().
 * @RETURN: The physical address if successful.
 *          0 on failure.
 */
